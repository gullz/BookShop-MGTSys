Public Class frmRetailerInvoice

    Dim xpos As Integer = 0


    Private Sub cmbCustomerName_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbRetailerName.SelectedIndexChanged
        Dim ds As DataSet
        ds = getData("RetailerID", "Retailer WHERE ShopName LIKE '%" + Trim(cmbRetailerName.Text) + "%'")
        txtRetailerID.Text = ds.Tables(0).Rows(0).Item(0).ToString
    End Sub


    Private Sub cmbCustomerName_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbRetailerName.TextChanged
        Dim ds As DataSet
        ds = getData("RetailerID,ShopName", "Retailer WHERE ShopName LIKE '%" + Trim(cmbRetailerName.Text) + "%'")
        Try
            For Each row In ds.Tables(0).Rows
                If Not cmbRetailerName.Items.Contains(row.Item(1).ToString) Then
                    cmbRetailerName.Items.Add(row.Item(1).ToString)
                End If
            Next
    ...
    Sub getNewOtherID()
        ...
        ConnectDB("BookShop")
        ds = getData("MAX(InvoiceNo) AS InvoiceNo", "Invoice WHERE InvoiceNo LIKE 'INR%'")
        Try
            txtInvoiceNo.Text = generateCode(ds.Tables(0).Rows(0).Item(0).ToString)
        Catch
            txtInvoiceNo.Text = "INR1"
        End Try
    End Sub
    ...
    Private Sub cmdAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdAdd.Click
        ...
        If btnEdit.Enabled = False Then
            ...
            updateData("RInvoice", "Quantity='" + txtQuantity.Text + "'", "InvoiceNo='" + txtInvoiceNo.Text + "' AND ItemNo='" + txtItemNo.Text + "'")
            MsgBox("Successfully Updated")
            If chkOrder.Checked Then
                setData("Orders", quote + txtRetailerID.Text + commawithquote + txtItemNo.Text + commawithquote + txtQuantity.Text + commawithquote + txtDate.Text + commawithquote + txtTotal.Text + quote)
                updateData("Inventory", "Quantity='" + Format(getInventory(txtItemNo.Text) - txtQuantity.Text) + "'", "ItemNo='" + txtItemNo.Text + "'")
            End If
            btnEdit.Enabled = True
            cmdAdd.Text = "Add"
        Else
            setData("Invoice", quote + txtInvoiceNo.Text + commawithquote + FormatDateTime(txtDate.Text, DateFormat.ShortDate) + commawithquote + "0" + commawithquote + txtTotalCost.Text + quote)
            setData("RInvoice", quote + txtInvoiceNo.Text + commawithquote + txtRetailerID.Text + commawithquote + Trim(txtItemNo.Text) + commawithquote + txtQuantity.Text + quote)
            If chkOrder.Checked Then
                If getInventory(txtItemNo.Text) >= txtQuantity.Text Then
                    setData("Orders", quote + txtRetailerID.Text + commawithquote + txtItemNo.Text + commawithquote + txtQuantity.Text + commawithquote + txtDate.Text + commawithquote + txtTotal.Text + commawithquote + "" + quote)
                    updateData("Inventory", "Quantity='" + Format(getInventory(txtItemNo.Text) - txtQuantity.Text) + "'", "ItemNo='" + txtItemNo.Text + "'")
                Else
    ...
    Sub refreshGrid()
        Dim ds As DataSet
        ds = getData("ItemNo,Description,UnitPrice,Quantity", "RetailerInvoice WHERE InvoiceNo='" + txtInvoiceNo.Text + "'")
    ...
    Sub updateTotal()
        Dim ds As DataSet
        ds = getData("SUM(UnitPrice*Quantity) as TotalCost", "RetailerInvoice WHERE RetailerID='" + txtRetailerID.Text + "' AND InvoiceNo='" + txtInvoiceNo.Text + "'")
        txtTotalCost.Text = ds.Tables(0).Rows(0).Item(0).ToString
    End Sub
    ...
    Private Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        If checkText() Then Return
        updateData("Invoice", "Total='" + txtTotalCost.Text + "'", "InvoiceNo='" + txtInvoiceNo.Text + "'")
        setData("Access", quote + login + commawithquote + FormatDateTime(Now, DateFormat.GeneralDate) + commawithquote + "Retailer Orders" + quote)

        MsgBox("Successfully saved")

    End Sub
    Sub loadInvoice()
        Dim supplierRep As New reptInvoiceRetail, strsupid As String, ds As DataSet
        strsupid = txtInvoiceNo.Text
        ds = getData("*", "RetailerInvoice WHERE InvoiceNo='" + strsupid + "'")
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        delData("RInvoice", "InvoiceNo='" + txtInvoiceNo.Text + "'")
    ...
    Sub getRetailer()
        Dim ds As DataSet
        ds = getData("*", "RetailerInvoice WHERE InvoiceNo='" + txtInvoiceNo.Text + "'")
        txtRetailerID.Text = ds.Tables(0).Rows(0).Item(0)
        cmbRetailerName.Text = ds.Tables(0).Rows(0).Item(1)
    End Sub
